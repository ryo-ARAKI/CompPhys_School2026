...
```text
手元の src/rhs.jl を埋める前に確認したいです。
現行の関数シグネチャ、引数、戻り値、型注釈に合わせて、
1. GKSL の各項と commutator! / dissipator! / rhs! の対応
2. A(t), γ, L_k, kshift の確認項目
3. 実装後に見る確認項目
の順で整理してください。
候補コードは必要最小限で示してください。
余裕があれば最後に、src/observables.jl の current_traces が何を集計しているかを 2〜3 行で補足してください。
```
